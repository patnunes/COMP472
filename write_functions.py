...
def write_solution(ctr, output_directory, execution_time, goal_node, total_cost, algo, heuristic=None):
    ...
    if execution_time >= 60:
        ...
        for node in solution_path:
            ...
            output_lines.append((str(node.moved_index)+' '+str(node.single_action_cost)+' '+str(node.state)).replace(
        ...
        for line in file:
            ...
            if algo == 'ucs':
                ...
                goal_node, total_cost, closedlist, execution_time = best_first_search(
                    puzzle, heuristic=heuristic, restrict_time=restrict_time)
            elif algo == 'astar':
                goal_node, total_cost, closedlist, execution_time = a_star(
