...
def analysis():
    ...
    with open(fifty_puzzle_file, 'w') as f:
        ...
        for x in range(puzzle_qty):
            ...
            f.write(str(template_puzzle).replace('[', '').replace(']', '').replace(
                ',', '').replace('\n', ' ').replace(',', '\n').strip() + '\n')
    f.close()
    ...
    write_analysis(output_directory, scaled_puzzle_file1, 'gbfs', 5, 3,
                   5, heuristic='h1', restrict_time=False, ctr='3x5')
    write_analysis(output_directory, scaled_puzzle_file2, 'gbfs', 3, 3,
                   5, heuristic='h1', restrict_time=False, ctr='3x3')
    write_analysis(output_directory, scaled_puzzle_file3, 'gbfs', 4, 4,
                   5, heuristic='h1', restrict_time=False, ctr='4x4')
